...
struct convert<std::vector<std::byte, Alloc> > {
    clmdep_msgpack::object const& operator()(clmdep_msgpack::object const& o, std::vector<std::byte, Alloc>& v) const {
        switch (o.type) {
...
struct pack<std::vector<std::byte, Alloc> > {
    ...
    clmdep_msgpack::packer<Stream>& operator()(clmdep_msgpack::packer<Stream>& o, const std::vector<std::byte, Alloc>& v) const {
...
struct object<std::vector<std::byte, Alloc> > {
    void operator()(clmdep_msgpack::object& o, const std::vector<std::byte, Alloc>& v) const {
        uint32_t size = checked_get_container_size(v.size());
...
struct object_with_zone<std::vector<std::byte, Alloc> > {
    void operator()(clmdep_msgpack::object::with_zone& o, const std::vector<std::byte, Alloc>& v) const {
        uint32_t size = checked_get_container_size(v.size());
        o.type = clmdep_msgpack::type::BIN;
        o.via.bin.size = size;
        if (size != 0) {
