...
struct convert<std::array<std::byte, N> > {
    clmdep_msgpack::object const& operator()(clmdep_msgpack::object const& o, std::array<std::byte, N>& v) const {
        switch (o.type) {
...
struct pack<std::array<std::byte, N> > {
    ...
    clmdep_msgpack::packer<Stream>& operator()(clmdep_msgpack::packer<Stream>& o, const std::array<std::byte, N>& v) const {
...
struct object<std::array<std::byte, N> > {
    void operator()(clmdep_msgpack::object& o, const std::array<std::byte, N>& v) const {
        uint32_t size = checked_get_container_size(v.size());
...
struct object_with_zone<std::array<std::byte, N> > {
    void operator()(clmdep_msgpack::object::with_zone& o, const std::array<std::byte, N>& v) const {
        uint32_t size = checked_get_container_size(v.size());
        o.type = clmdep_msgpack::type::BIN;
        o.via.bin.size = size;
        if (size != 0) {
