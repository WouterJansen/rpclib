...
#define MSGPACK_ADAPTOR_CONVERT_SPAN_BINARY(byte_t)                                                                 \
    ...
    struct convert<std::span<const byte_t> > {                                                                      \
        clmdep_msgpack::object const& operator()(clmdep_msgpack::object const& o, std::span<const byte_t>& v) const {             \
            switch (o.type) {                                                                                       \
...
#define MSGPACK_ADAPTOR_PACK_SPAN_BINARY(byte_t)                                                                    \
    ...
    struct pack<std::span<byte_t> > {                                                                               \
        ...
        clmdep_msgpack::packer<Stream>& operator()(clmdep_msgpack::packer<Stream>& o, const std::span<byte_t>& v) const {         \
...
#define MSGPACK_ADAPTOR_OBJECT_SPAN_BINARY(byte_t)                                                                  \
    ...
    struct object<std::span<byte_t> > {                                                                             \
        void operator()(clmdep_msgpack::object& o, const std::span<byte_t>& v) const {                                     \
            uint32_t size = checked_get_container_size(v.size());                                                   \
...
#define MSGPACK_ADAPTOR_OBJECT_WITH_ZONE_SPAN_BINARY(byte_t)                                                        \
    ...
    struct object_with_zone<std::span<byte_t> > {                                                                   \
        void operator()(clmdep_msgpack::object::with_zone& o, const std::span<byte_t>& v) const {                          \
            uint32_t size = checked_get_container_size(v.size());                                                   \
            o.type = clmdep_msgpack::type::BIN;                                                                            \
            o.via.bin.size = size;                                                                                  \
            if (size != 0) {                                                                                        \
